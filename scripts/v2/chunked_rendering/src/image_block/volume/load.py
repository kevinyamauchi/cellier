"""LOAD level selection for bricked volume rendering.

Phase 3 (optimised): two-level speedup over the previous array pipeline.

Optimisation 1 — enumerate coarse grids directly, no deduplication
-------------------------------------------------------------------
Previous approach: enumerate all 32 768 base-grid cells, compute levels,
then deduplicate with ``np.unique`` (O(N log N) sort on all N cells).

New approach: for each LOAD level k enumerate *only that level's coarse
grid* (e.g. 512 bricks for L3 on a 32³ base grid), filter by its
distance band, and concatenate.  No two levels can produce the same
coarse brick, so **no deduplication is ever needed**.  The sort is
eliminated entirely.

Optimisation 2 — precomputed, cached grid data
-----------------------------------------------
The coarse grid index arrays and world-space brick centres depend only on
``block_size`` and ``grid_dims``, which are fixed at startup.  They are
computed once by ``build_level_grids()`` and stored on ``BlockVolumeState``
as ``_level_grids``.  The hot path (``select_levels_from_cache``) does no
allocation — only distance computation and boolean masking on the cached
arrays.
"""
...
# ---------------------------------------------------------------------------
# Hot path: per-Update LOAD selection using the cache
# ---------------------------------------------------------------------------
...
def select_levels_from_cache(
    ...
    base_layout: BlockLayout | None = None,
) -> np.ndarray:
    """Select LOAD levels using precomputed coarse grid data.

    For each level k, compute distances from the camera to the precomputed
    coarse brick centres, apply the distance band that belongs to level k,
    and collect survivors.  No allocation of new coordinate arrays occurs;
    only distance computation and boolean masking on the cached arrays.

    Because the bands partition the distance axis, no brick can appear in
    more than one level — **no deduplication needed**.
    ...
    thresholds : list[float] or None
        LOAD cutoff distances.  ``thresholds[i]`` is the distance beyond which
        level ``i+2`` is preferred over level ``i+1``.
        If None, all bricks are assigned level 1.
    base_layout : BlockLayout or None
        Only used to compute default thresholds when ``thresholds`` is None.
    ...
    for level in range(1, n_levels + 1):
        ...
        # Distance from camera to each coarse brick centre.
        diff = centres - cam                       # (M_k, 3)
        dist = np.sqrt((diff * diff).sum(axis=1))  # (M_k,)  — avoids (M_k,3) temp

        # Distance band for level k:
        #   level 1  : dist <  thresholds[0]
        #   level k  : thresholds[k-2] <= dist < thresholds[k-1]
        #   level n  : dist >= thresholds[n-2]
        if level == 1:
            mask = dist < thresholds[0] if thresholds else np.ones(len(dist), dtype=bool)
        elif level == n_levels:
...
def select_levels_arr_forced(
    ...
    """Return the full coarse grid for a forced single LOAD level.
    ...
    # Fallback: compute from scratch (used before cache is built or in tests)
    gd, gh, gw = base_layout.grid_dims
    ...
    gz_c = gz_c.ravel(); gy_c = gy_c.ravel(); gx_c = gx_c.ravel()
    lvl = np.full(len(gz_c), force_level, dtype=np.int32)
    return np.stack([lvl, gz_c, gy_c, gx_c], axis=1)


# ---------------------------------------------------------------------------
# Sort (unchanged — operates on the small post-selection array)
# ---------------------------------------------------------------------------
...
def sort_arr_by_distance(
    ...
    Centres are always recomputed from the array columns — O(M) and faster
    than an O(K×M_k) index-match against the full cached grid.
    ``level_grids`` is accepted for API compatibility but unused.
    ...
    scales = np.left_shift(1, (levels - 1)).astype(np.float64)  # 2^(level-1)
    bw = float(block_size) * scales

    # World-space centres  (x=W, y=H, z=D)
...
# ---------------------------------------------------------------------------
# Convert to BlockKey dict  (called once, on the small final array)
# ---------------------------------------------------------------------------


def arr_to_brick_keys(arr: np.ndarray) -> dict[BlockKey, int]:
    """Convert a brick array to ``dict[BlockKey, int]``.
    ...
    for row in arr:
        level = int(row[0])
        key = BlockKey(level=level, gz=int(row[1]), gy=int(row[2]), gx=int(row[3]))
        required[key] = level
    return required


# ---------------------------------------------------------------------------
# Legacy dict-based API  (delegates to array pipeline, for compatibility)
# ---------------------------------------------------------------------------


def select_levels_arr(
    base_layout: BlockLayout,
    n_levels: int,
    camera_pos: np.ndarray,
    thresholds: list[float] | None = None,
) -> np.ndarray:
    """Legacy entry-point — builds a temporary cache and delegates."""
    grids = build_level_grids(base_layout, n_levels)
    return select_levels_from_cache(grids, n_levels, camera_pos, thresholds, base_layout)
...
def select_levels(
    ...
    """Assign LOAD levels — legacy dict API."""
    arr = select_levels_arr(base_layout, n_levels, camera_pos, thresholds)
    return arr_to_brick_keys(arr)
...
def sort_by_distance(
    ...
    """Sort nearest-first — legacy dict API."""
    if not required:
        return required
    keys = list(required.keys())
    arr = np.array([[k.level, k.gz, k.gy, k.gx] for k in keys], dtype=np.int32)
    sorted_arr = sort_arr_by_distance(arr, camera_pos, block_size)
    return {
        BlockKey(level=int(r[0]), gz=int(r[1]), gy=int(r[2]), gx=int(r[3])): int(r[0])
        for r in sorted_arr
    }
