...
from ``load.py`` and the vectorised corner construction from plan section 2.2,
...
compute_brick_aabb_corners   Convert a BlockKey to 8 world-space AABB corners.
bricks_in_frustum            Conservative AABB test; returns (visible_keys, timings).

Axis-order note
---------------
BlockKey stores (gz, gy, gx) in DHW / numpy order, but world coordinates
...
from image_block.volume.load import CORNER_OFFSETS
...
def bricks_in_frustum(
    brick_keys: set[BlockKey] | dict[BlockKey, int],
    layout: BlockLayout,
    block_size: int,
    frustum_planes: np.ndarray,
) -> tuple[dict[BlockKey, int], dict]:
    ...
    Parameters
    ----------
    brick_keys : set[BlockKey] or dict[BlockKey, int]
...
def bricks_in_frustum_arr(
    ...
    Vectorised; operates entirely on numpy arrays — no BlockKey objects.
