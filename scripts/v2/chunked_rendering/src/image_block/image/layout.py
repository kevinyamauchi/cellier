"""Block layout parameters for 2D tiled image rendering.

Computes all derived layout constants from the image shape and block size.
One ``BlockLayout2D`` is created per scale level; ``layouts[0]`` is the
finest.
"""
...
if __name__ == "__main__":
    ...
    print(f"BlockLayout2D: shape={layout.volume_shape}")
    print(f"  grid_dims={layout.grid_dims}")
    print(f"  padded_shape={layout.padded_shape}")
    print(f"  n_tiles={layout.n_tiles}")
    print(f"  block_size={layout.block_size}  overlap={layout.overlap}")
    print("BlockLayout2D OK")
