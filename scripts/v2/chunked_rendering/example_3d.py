...
LOAD bias spinbox       — scale LOAD thresholds
...
from image_block import BlockState3D, open_ts_stores
...
GPU_BUDGET = (
    1024 * 1024**2
)  # 1 GiB — fits all L3 (512) + all L2 (4096) bricks for a 1024³ volume
...
class BlockVolumeApp(QMainWindow):
    ...
    def __init__(self, state: BlockState3D, vol: gfx.Volume) -> None:
        super().__init__()
        self._state = state
        self._vol = vol
    ...
    def _setup_ui(self) -> None:
        ...
        # LOAD bias spinbox
        panel_layout.addWidget(QLabel("LOAD bias:"))
        self._load_bias_sb = QDoubleSpinBox()
        self._load_bias_sb.setRange(0.1, 10.0)
        self._load_bias_sb.setSingleStep(0.1)
        self._load_bias_sb.setDecimals(2)
        self._load_bias_sb.setValue(LOAD_BIAS)
        self._load_bias_sb.setToolTip(
            "Scales LOAD distance thresholds.\n"
            "> 1: coarser LOAD sooner (fewer bricks)\n"
            "< 1: finer LOAD longer (more bricks)"
        )
        panel_layout.addWidget(self._load_bias_sb)
    ...
    async def _on_update_clicked(self) -> None:
        ...
        load_bias = self._load_bias_sb.value()  # captured at submission time
        focal_half_height = (screen_h / 2) / np.tan(fov_y_rad / 2)
        thresholds = [
            (2 ** (k - 1)) * focal_half_height / load_bias
    ...
    def _print_update_summary(
        ...
        for name, p in zip(plane_names, planes, strict=False):
        ...
        print()
        print(
            f"load assignment  (force_level={force}  load_bias={self._load_bias_sb.value():.2f})"
        )
...
async def async_main(state: BlockState3D, vol: gfx.Volume) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Phase 3 async multi-LOAD volume viewer"
    )
    parser.add_argument(
        "--make-files",
        action="store_true",
