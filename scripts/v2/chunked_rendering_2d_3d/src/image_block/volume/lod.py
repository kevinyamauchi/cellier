...
New approach: for each LOD level k enumerate *only that level's coarse
...
from image_block.volume.tile_manager import BrickKey
...
# ---------------------------------------------------------------------------
# Hot path: per-Update LOD selection using the cache
# ---------------------------------------------------------------------------
...
def select_levels_from_cache(
    ...
    thresholds : list[float] or None
        LOD cutoff distances.  ``thresholds[i]`` is the distance beyond which
...
# ---------------------------------------------------------------------------
# Convert to BrickKey dict  (called once, on the small final array)
# ---------------------------------------------------------------------------


def arr_to_brick_keys(arr: np.ndarray) -> dict[BrickKey, int]:
    """Convert a brick array to ``dict[BrickKey, int]``.
    ...
    required : dict[BrickKey, int]
        ``{BrickKey: level}`` preserving row order.
    """
    required: dict[BrickKey, int] = {}
    for row in arr:
        level = int(row[0])
        key = BrickKey(level=level, gz=int(row[1]), gy=int(row[2]), gx=int(row[3]))
        required[key] = level
    return required
...
def select_levels(
    ...
    """Assign LOD levels — legacy dict API."""
...
def sort_by_distance(
    ...
    return {
        BrickKey(level=int(r[0]), gz=int(r[1]), gy=int(r[2]), gx=int(r[3])): int(r[0])
        for r in sorted_arr
    }
