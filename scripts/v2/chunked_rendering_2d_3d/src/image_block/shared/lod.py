...
from image_block.shared.tile_manager import TileKey
...
# ---------------------------------------------------------------------------
# Hot path: per-Update LOD selection
# ---------------------------------------------------------------------------
...
def select_lod_2d(
    ...
    """Select LOD levels based on the camera zoom.

    The 2D selector uses a single global LOD level for all tiles based
    ...
    --------------------------------------------------------
    Compute the continuous LOD level from the ratio of screen resolution
    to data resolution:
    ...
    ``lod_bias`` is multiplicative, matching the 3D volume viewer:
        - lod_bias = 1.0  standard mipmapping (default, neutral)
        - lod_bias > 1.0  prefer coarser levels (acts as if screen pixels
          were larger than they really are)
        - lod_bias < 1.0  prefer finer levels (acts as if screen pixels
          were smaller)

    Doubling the bias shifts every transition by one full LOD level.
    ...
    # Apply multiplicative bias: lod_bias > 1 makes the screen pixel
...
def arr_to_tile_keys(arr: np.ndarray) -> dict[TileKey, int]:
    """Convert array rows to a TileKey dict.
    ...
    required : dict[TileKey, int]
        ``{TileKey: level}`` preserving row order.
    """
    required: dict[TileKey, int] = {}
    for row in arr:
        level = int(row[0])
        key = TileKey(level=level, gy=int(row[1]), gx=int(row[2]))
        required[key] = level
    return required
...
if __name__ == "__main__":
    ...
    arr = select_lod_2d(grids, 3, camera_info, lod_bias=1.0)
