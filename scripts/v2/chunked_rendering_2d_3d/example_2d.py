...
def make_zarr_sample(path: pathlib.Path) -> None:
    ...
    for name, factor in [("s0", 1), ("s1", 2), ("s2", 4)]:
        if factor > 1:
            data = block_reduce(
                base, (factor, factor, factor), np.max
            ).astype(np.float32)
        else:
            data = base
...
def make_aabb_wireframe(
    w: float, h: float, color: tuple
) -> gfx.Line:
...
def make_viewport_wireframe(
    ...
    positions = np.array(
        [
            [x0, y0, 0.1], [x1, y0, 0.1],   # bottom
            [x1, y0, 0.1], [x1, y1, 0.1],   # right
            [x1, y1, 0.1], [x0, y1, 0.1],   # top
            [x0, y1, 0.1], [x0, y0, 0.1],   # left
        ],
        dtype=np.float32,
    )
    colors = np.array([color] * 8, dtype=np.float32)
...
class ImageBlockApp(QWidget):
    ...
    def __init__(
        ...
        # LOD bias spinbox (multiplicative: 1.0 = neutral, matching 3D viewer).
        panel.addWidget(QLabel("LOD bias:"))
        self._lod_bias_sb = QDoubleSpinBox()
        self._lod_bias_sb.setRange(0.1, 10.0)
        self._lod_bias_sb.setSingleStep(0.1)
        self._lod_bias_sb.setValue(1.0)
        panel.addWidget(self._lod_bias_sb)
    ...
    async def _on_update_clicked(self) -> None:
        ...
        if self._load_task is not None and not self._load_task.done():
            ...
            task_age_ms = (
                time.perf_counter() - self._load_task_start_time
            ) * 1000
        ...
        self._print_update_summary(
            camera_info, stats, t_planning_ms,
            prev_was_cancelled, task_age_ms,
        )
    ...
    def _print_update_summary(
        ...
        print(f"  logical_size:    {camera_info['viewport_width']:.0f} x "
        ...
        print(f"  view_min:        [{camera_info['view_min'][0]:.1f}, "
              f"{camera_info['view_min'][1]:.1f}]")
        print(f"  view_max:        [{camera_info['view_max'][0]:.1f}, "
        ...
        print("cache budget")
        drop_flag = (
            f"  *** {n_dropped} DROPPED (nearest-first) ***"
            if n_dropped
            else ""
        )
...
async def async_main(state: BlockState2D, image: gfx.Image) -> None:
    ...
    window.setWindowTitle(
        "Chunked 2D Image Viewer -- LUT Indirection (async)"
    )
    window.show()
