...
Force-level radios (3D) — override automatic LOD selection
LOD bias spinbox (3D)  — scale LOD distance thresholds
...
from image_block import make_block_image, open_ts_stores, BlockState2D
from image_block.volume import make_block_volume, BlockVolumeState
...
GPU_BUDGET_3D = 512 * 1024**2   # 512 MiB  (3D brick cache)
...
def make_multiscale_zarr(zarr_path: pathlib.Path) -> None:
    ...
    vol = binary_blobs(
        length=base_length, n_dim=3,
        blob_size_fraction=0.2, volume_fraction=0.2,
    ).astype(np.float32)
...
class CombinedApp(QMainWindow):
    ...
    def _setup_scenes(self) -> None:
        ...
        self._camera_2d.show_object(
            self._scene_2d, view_dir=(0, 0, -1), up=(0, 1, 0)
        )
        ...
        self._camera_3d.show_object(
            self._scene_3d, view_dir=(-1, -1, -1), up=(0, 0, 1)
        )
    ...
    def _setup_ui(self) -> None:
        ...
        lbl_level = QLabel("Force LOD level:")
        ...
        lbl_lod = QLabel("LOD bias:")
        pl.addWidget(lbl_lod)
        self._widget_3d.append(lbl_lod)

        self._lod_bias_sb = QDoubleSpinBox()
        self._lod_bias_sb.setRange(0.1, 10.0)
        self._lod_bias_sb.setSingleStep(0.1)
        self._lod_bias_sb.setValue(LOD_BIAS_DEFAULT)
        pl.addWidget(self._lod_bias_sb)
        self._widget_3d.append(self._lod_bias_sb)
    ...
    async def _update_2d(self) -> None:
        ...
        lod_bias = LOD_BIAS_DEFAULT  # extend with a spinbox if desired
        ...
        print(f"  view_center:    ({camera_info['view_center'][0]:.1f}, "
        ...
        if not fill_plan:
            self._status_label.setText(f"2D: all {stats.get('total_required',0)} tiles cached")
            return
    ...
    async def _update_3d(self) -> None:
        ...
        print(sep)
        print(f"  cam_pos:        ({cam_pos[0]:.1f}, {cam_pos[1]:.1f}, {cam_pos[2]:.1f})")
        print(f"  thresholds:     {[f'{t:.0f}' for t in thresholds]}")
        print(f"  lod_bias:       {lod_bias:.2f}")
        print(f"  frustum cull:   {'ON' if culling_on else 'OFF'}")
        if culling_on:
            print(f"  total_bricks:   {stats.get('total_required','?')}")
            print(f"  visible:        {stats.get('total_required',0) - stats.get('n_culled',0)}")
            print(f"  culled:         {stats.get('n_culled',0)}")
        ...
        if not fill_plan:
            self._status_label.setText(f"3D: all {stats.get('total_required',0)} bricks cached")
            return
