...
class EventBus:
    ...
    @property
    def visual_model_signals(self) -> dict[str, SignalInstance]:
    ...
    @property
    def visual_control_signals(self) -> dict[str, SignalInstance]:
    ...
    def register_visual(self, visual: VisualType):
        ...
        """
        if visual.id in self.visual_model_signals:
        ...
        self.visual_model_signals[visual.id] = SignalInstance(
    ...
    def subscribe_to_visual(self, visual_id: str, callback: Callable):
        ...
        """
        try:
            visual_model_signal = self.visual_model_signals[visual_id]
        except KeyError:
    ...
    def register_visual_controls(self, visual_id: str, signal: SignalInstance):
        ...
        """
        if visual_id not in self.visual_control_signals:
            # If we haven't registered this visual_id yet, create the signal
            self.visual_control_signals[visual_id] = SignalInstance(
                name=visual_id,
    ...
    def subscribe_to_visual_controls(self, visual_id: str, callback: Callable):
        """Subscribe to the event emitted when the visual controls are updated."""
        try:
            visual_control_signal = self.visual_control_signals[visual_id]
        except KeyError:
            raise ValueError(f"Visual {visual_id} is not registered.") from None
    ...
    def _on_visual_model_update(self, event: Any | None = None):
        ...
        try:
            signal = self.visual_model_signals[emitter_object.id]
        except KeyError:
            logger.debug(
    ...
    def _on_visual_control_update(self, event: Any | None = None):
        ...
        visual_id = event["id"]
        try:
            control_signal = self.visual_control_signals[visual_id]
        except KeyError:
            logger.debug(
        ...
        if visual_id in self._visual_control_signals:
            ...
            visual_signal = self.visual_model_signals[visual_id]
