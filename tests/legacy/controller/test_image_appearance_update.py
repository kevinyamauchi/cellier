...
from cellier._legacy.models.visuals import ImageAppearance, MultiscaleImageVisual
...
def test_labels_appearance_update(qtbot):
    ...
    image_material = ImageAppearance(color_map="glasbey", visible=True)
    image_visual = MultiscaleImageVisual(
        name="image_visual",
    ...
    # make the scene
    scene = Scene(
        dims=dims_manager, visuals=[image_visual], canvases={canvas.id: canvas}
    )
    ...
    image_renderer_implementation = viewer_controller._render_manager._visuals[
        image_visual.id
    ]

    # check that the labels visual is visible
    assert image_renderer_implementation.node.visible

    # set the labels to not visible and verify
    image_visual.appearance.visible = False
    assert not image_renderer_implementation.node.visible
